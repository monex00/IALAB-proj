...
applicabile(tp, pos(Riga,Colonna)):-
    wormhole(pos(Riga,Colonna), _).
...
% trasforma(Az, S0, S1)
trasforma(tp,pos(Riga,Colonna),pos(RigaDest,ColonnaDest)):-
    wormhole(pos(Riga,Colonna),pos(RigaDest,ColonnaDest)).
